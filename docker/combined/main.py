...
def main():
    ...
    sdm = SDM630(device=env["SERIAL_DEVICE"], baud=int(env["SERIAL_BAUDRATE"]), parity="N", timeout=5, unit=int(env["SDM_ADDR"]))
    ...
    sdm_obs = get_sdm_energy_values_observable(sdm, float(env["SDM_INTERVAL"]), serial_lock, scheduler)
